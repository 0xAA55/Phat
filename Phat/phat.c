...
PHAT_FUNC static uint32_t Phat_CRC32(uint32_t crc, void *buf, size_t size)
{
	uint8_t *ptr = buf;
	crc = ~crc;
	for(size_t i = 0; i < size; i++) crc = crc_table[(crc ^ ptr[i]) & 0xFF] ^ (crc >> 8);
	return ~crc;
}
...
PHAT_FUNC static PhatState Phat_InvalidateCachedSector(Phat_p phat, Phat_SectorCache_p cached_sector)
{
	...
	{
		if (!phat->write_enable && !Phat_IsCachedSectorSync(cached_sector)) return PhatState_InternalError;
...
Ended:
	if (lfn_item->order & 0x40)
	{
		buffer->LFN_length = write_pos % 13 ? write_pos - 1 : write_pos;
		if (buffer->LFN_length > 0 && !buffer->LFN_name[buffer->LFN_length - 1]) buffer->LFN_length--;
	}
	return PhatState_OK;
}
...
PHAT_FUNC PhatState Phat_InitializeMBR(Phat_p phat, PhatBool_t force, PhatBool_t flush)
{
	...
	const uint8_t MBR[512] =
		...
		"\x65\x6D\x00\x00\x00\x63\x7B\x9A\x1D\x41\xF8\xC7\x00\x00\x00\x00"
	...
	memcpy(cached_sector->data, MBR, 512);
	Phat_SetCachedSectorModified(cached_sector);
	if (flush)
	{
		ret = Phat_WriteBackCachedSector(phat, cached_sector);
...
PHAT_FUNC PhatState Phat_MakeFS_And_Mount(Phat_p phat, int partition_index, int FAT_bits, uint16_t root_dir_entry_count, uint32_t volume_ID, const char *volume_lable, PhatBool_t flush)
{
	...
	static const uint8_t dbr12_boot_code[448] =
	...
	static const uint8_t dbr16_boot_code[448] =
	...
	static const uint8_t dbr32_boot_code[420] =
	...
	case 12:
		...
		break;
	case 16:
		// Choose `sectors_per_cluster` based on the partition size
		sectors_per_cluster = 1;
		for (uint64_t i = 64 * 1024 * 1024 / 512; i < partition_size_in_sectors; i <<= 1)
		{
			sectors_per_cluster <<= 1;
		...
		break;
	case 32:
		// Choose `sectors_per_cluster` based on the partition size, prefer 8
		sectors_per_cluster = 8;
		if (partition_size_in_sectors >= 1024 * 1024 * 1024 / 512)
		{
		...
		if (partition_size_in_sectors <= 4 * 1024 / 512) sectors_per_cluster = 4;
		if (partition_size_in_sectors <= 2 * 1024 / 512) sectors_per_cluster = 2;
		if (partition_size_in_sectors <= 1 * 1024 / 512) sectors_per_cluster = 1;
	}
	...
	free_clusters = ((SLBA_t)partition_size_in_sectors - (SLBA_t)end_of_FAT_LBA) / (SLBA_t)sectors_per_cluster - 2 - 1;
	if (free_clusters <= 0) return PhatState_PartitionTooSmall;
	...
	if (FAT_bits != 32)
	{
		...
		case 12:
			...
			memcpy(dbr->boot_code, dbr12_boot_code, 448);
			phat->end_of_cluster_chain = 0xFF8;
			break;
		case 16:
			memcpy(dbr->file_system_type, "FAT16   ", 8);
			memcpy(dbr->boot_code, dbr16_boot_code, 448);
	...
	else
	{
		...
		memcpy(dbr->boot_code, dbr32_boot_code, 420);
		dbr->boot_sector_signature = 0xAA55;
